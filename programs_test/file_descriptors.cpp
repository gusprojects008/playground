...
int main() {
    // return an integer related to a file descriptor that teh kernel has registered in the process's FD table, and which corresponds tothe structure that the kernel has reserved for it in its global table of structures corresponding to file descriptors opened and used by the system and processes. This makes it possible for the program to peform operations on devices, file or sockets. Communicationg with them through syscalls for these file descriptors.
